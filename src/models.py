...
def credit_crunch(data_package, return_evaluation=False, basic_model=False):
    ...
    # import train data
    raw_data = pd.read_csv("src/data/Credit_Data_Raw.csv")
    ...
    # creating dynamic model based on passed data bundle
        ...
        for layer in np.arange(0, number_hidden_layers):
            model.add(
                keras.layers.Dense(
                    units=number_hidden_nodes, activation=layer_activation
                )
            )

        # adding classifier layer
        model.add(
            keras.layers.Dense(units=number_classes, activation=classifier_activation)
        )
